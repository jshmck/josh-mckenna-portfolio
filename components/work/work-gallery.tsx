  ...
  return (
    ...
    <div className="contents" onClickCapture={handleGalleryLinkClick}>
      ...
      <div
        ...
        key={entranceRun}
        ref={pillRowRef}
        // max-md:min-h-[102px] + content-start: below md the row is 2
        // lines with search closed and 3 with it open — the open bar
        // doesn't fit the space left on the second line, so flex-wrap
        // drops it to a third. That wrap is the DESIRED behaviour ("it's
        // ok if you hit the search and it goes to the third row," per
        // Josh, closing a long loop: an overlay version that grew the
        // bar over the neighbouring pills to avoid the wrap entirely
        // was rejected — "i dont like the search covering other
        ...
        // sizing comment for why these boxes don't reduce to clean
        // math.
      ...
      >
        ...
        {/* Search rides the same woosh, last in the stagger — "at the
            very end i want there to be a search bar," per Josh. Plain
            in-flow flex item, deliberately: below md, the closed circle
            sits inline after the last category pill; opening grows the
            input past what's left of that line and flex-wrap drops the
            bar to the row's (pre-reserved, see above) third line;
            closing shrinks it and it rejoins the end of the pill row —
            "it's ok if you hit the search and it goes to the third row
            (or next row) just make sure when you're not using it, it
            bounces back to small pill at the end of the row," per Josh.
            That settles a three-way tug-of-war recorded here so none of
            the rejected shapes come back: a basis-full break that
            parked search on its own line even while closed ("it should
            be at the end of the pills"), then a fixed-footprint overlay
            that grew the open bar leftward OVER the neighbouring pills
            to dodge the wrap ("i dont like the search covering other
            pills") — the natural wrap was never the problem, a pill
            that failed to RETURN was (the stuck-:hover width bug, see
            pointer-fine on the input below). */}
        <span
